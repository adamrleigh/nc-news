...
export const SingleComment = ( {comment, isProfile, setComments, setCommentCount} ) => {
    ...
    const [commentLikes, setCommentLikes] = useState(comment && comment.votes || 0);
    ...
    useEffect(async () => {
        if (comment && isProfile) {
            const {article: commentArticle} = await fetchArticleById(comment.article_id);
    ...
    return (
        ...
        <LinkContainer to={`/users/${comment.author}`}>
            <Card.Header>
                @{comment.author}
                <br></br>
                <img src={commenter.avatar_url} style={{width: "40px", height: "40px"}}></img>
                </Card.Header>
        </LinkContainer>
}        
        <Card.Body>
